...
STROKE_PROFILES = {
    "fountain": {"cn": "钢笔",   "passes": 2, "jitter_k": 1.00, "width_k": 1.10, "alpha": 248, "taper": True,  "press": 0.58, "over": 1.10, "grain": 0.05, "blur": 0.25},
    "pencil":   {"cn": "铅笔",   "passes": 3, "jitter_k": 1.25, "width_k": 1.05, "alpha": 234, "taper": True,  "press": 0.74, "over": 0.75, "grain": 0.15, "blur": 0.35},
    "brush":    {"cn": "毛笔",   "passes": 2, "jitter_k": 1.35, "width_k": 1.60, "alpha": 250, "taper": True,  "press": 1.00, "over": 1.45, "grain": 0.08, "blur": 0.45},
    "marker":   {"cn": "马克笔", "passes": 1, "jitter_k": 0.55, "width_k": 2.00, "alpha": 255, "taper": False, "press": 0.00, "over": 0.00, "grain": 0.03, "blur": 0.18},
    "charcoal": {"cn": "炭笔",   "passes": 3, "jitter_k": 1.90, "width_k": 1.85, "alpha": 216, "taper": True,  "press": 0.85, "over": 1.00, "grain": 0.28, "blur": 0.65},
}
...
class Pen:
    ...
    def __init__(self, seed=0, scale=1.0, roughness=1.0, bowing=1.0,
        ...
        self.img = None            # 绑定画布，供 shade() 铺排线（v2.4.0）
        self.shade_on = True       # 体量排线总开关（v2.4.0）
    ...
    def line(self, d, pts, color, w=3.0, jitter=None, passes=None, alpha=None, taper=None):
        ...
        if taper is None:
            press = prof.get("press", 1.0 if prof.get("taper") else 0.0)
        else:
            press = 1.0 if taper else 0.0
        over = prof.get("over", 0.0)
        ...
        for p in range(n_pass):
            ...
            if over > 0:
                jpts = self._overshoot(jpts, over * self.scale)
            aa = a0 if p == 0 else max(48, a0 - 95)
            ww = max(1, int(round(w0 * (1.0 if p == 0 else 0.62))))
            if press > 0:
                self._taper(d, jpts, color, ww, aa, press)
            else:
                d.line(jpts, fill=(*color, aa), width=ww, joint="curve")

    def _taper(self, d, pts, color, wmax, alpha, press=1.0):
        """压感笔触：起笔轻、行笔重、收笔出锋，并叠行笔速度带来的低频起伏。
        press 为压感强度（0 等宽，1 最强）；小图元自动减弱，避免短线被压断。"""
        n = len(pts)
        if n < 2:
            return
        xs = [p[0] for p in pts]
        ys = [p[1] for p in pts]
        if max(max(xs) - min(xs), max(ys) - min(ys)) < 44:
            press *= 0.35                                  # 短元素近乎等宽
        ph = self.rng.uniform(0, 2 * math.pi)
        ripple = self.rng.uniform(0.30, 0.80)
        for i in range(n - 1):
            t = i / (n - 1)
            env = 0.20 + 0.80 * math.sin(math.pi * min(1.0, t * 1.02)) ** 0.72
            wave = 1.0 + ripple * 0.26 * math.sin(2 * math.pi * t * 1.35 + ph)
            k = (1 - press) + press * env * wave
            w = max(1, int(round(wmax * k)))
            a = int(max(64, min(alpha, alpha * (0.74 + 0.26 * env))))   # 收笔略淡
            d.line([pts[i], pts[i + 1]], fill=(*color, a), width=w)

    def _overshoot(self, pts, amount):
        """出锋：开放路径两端沿切线各延长一小段，模拟手绘的果断过笔；闭合路径不出锋。"""
        if amount <= 0 or len(pts) < 3:
            return pts
        head, tail = pts[0], pts[-1]
        if math.hypot(head[0] - tail[0], head[1] - tail[1]) < amount * 4:
            return pts

        def ext(a, b):
            dx, dy = b[0] - a[0], b[1] - a[1]
            l = math.hypot(dx, dy) or 1.0
            return (b[0] + dx / l * amount, b[1] + dy / l * amount)

        return [ext(pts[1], head)] + list(pts) + [ext(pts[-2], tail)]
    ...
    # ---------- 体量排线（v2.4.0） ----------
    def bind(self, img):
        """绑定画布，使 shade() 能在母题内部直接铺排线。"""
        self.img = img
        return self

    def shade(self, shape_pts, angle=42, gap=13, alpha=50, cross=False):
        """给一个面铺浅斜排线，增加画面体量；开关由 shade_on 控制。"""
        if not self.shade_on or self.img is None or len(shape_pts) < 3:
            return
        self.fill_hachure(self.img, shape_pts, angle=angle, gap=gap,
                          color=self.pal["ink2"], alpha=alpha, cross=cross)

    # ---------- 排线填充 ----------
    def fill_hachure(self, img, shape_pts, angle=45, gap=14, color=None, alpha=125,
                     cross=False, dots=False, use_pen=True):
        """排线填充。use_pen=True（默认）时每条排线走手绘笔，自带抖动、压感与收锋；
        use_pen=False 退回等宽机械排线（旧行为）。"""
        ...
        if use_pen:
            ...
            for i in range(-k, k + 1):
                bx, by = cx + px * i * gap, cy + py * i * gap
                jx, jy = self.rng.uniform(-2.8, 2.8), self.rng.uniform(-2.8, 2.8)
                p0 = (bx - dx * span + jx, by - dy * span + jy)
                p1 = (bx + dx * span + jx, by + dy * span + jy)
                aa = int(max(28, min(255, alpha * self.rng.uniform(0.70, 1.12))))   # 每条浓淡不一
                self.line(ld, [p0, p1], color, 1.6, jitter=1.35, passes=1, alpha=aa, taper=0.5)
            mask = Image.new("L", img.size, 0)
            ImageDraw.Draw(mask).polygon(shape_pts, fill=255)
            layer.putalpha(ImageChops.multiply(layer.getchannel("A"), mask))
            img.alpha_composite(layer)
            if cross:
                self.fill_hachure(img, shape_pts, angle + 90, gap, color, int(alpha * 0.75),
                                  False, False, True)
            return
...
def s_plant(d, W, H, pen, ink):
    ...
    pen.shade([(x0 + 4, wy + 4), (x1 - 4, wy + 4), (x1 - 4, y1 - 4), (x0 + 4, y1 - 4)],
              angle=38, gap=15, alpha=46)                 # 水体斜排线
    for i in range(6):
...
def s_archive(d, W, H, pen, ink):
    ...
    pen.shade([(x0 + 5, H * 0.245), (x1 - 5, H * 0.245), (x1 - 5, H * 0.795), (x0 + 5, H * 0.795)],
              angle=58, gap=18, alpha=40)                  # 柜体浅排线
    for k in range(3):
