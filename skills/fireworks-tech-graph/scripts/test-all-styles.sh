...
TEST_DIR="${TEST_DIR:-${SKILL_DIR}/test-output}"
...
for i in "${!STYLES[@]}"; do
    ...
    # Check if style reference exists（按编号匹配 style-N-*.md）
    STYLE_FILE=$(ls "${SKILL_DIR}"/references/style-${STYLE}-*.md 2>/dev/null | head -n 1 || true)
